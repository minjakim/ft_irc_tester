...
void connect_client_to_server(char *ip)
{
    // 클라이언트를 서버에 연결
...
int initialize(int argc, char **argv)
{
    if (argc < 3)
    {
        std::cerr << argv[0] << " <filename> <ip>" << std::endl;
        return 1;
    }
...
int main(int argc, char **argv)
{
	if (initialize(argc, argv))
		return 1;
	connect_client_to_server(argv[2]);
